...
with tab1:
    ...
    modelnames = ['decision_tree_model.pkl', 'logistic_regression_model.pkl', 'random_forest_model.pkl', 'svm_model.pkl', 'best_random_forest.pkl']
...
with tab2:
    ...
    st.info("""
        ...
        4. Feature values conventions: \n
            ...
            - RestingECG: resting electrocardiogram results [0:Normal, 1:having ST_T wave abnormality (T wave inversions and/or ST elevation or depression of >0.05 mV), 2:showing probable or definite left ventricular hypertrophy by Estes' criteria] \n
...
""")
    ...
    if uploaded_file is not None:
        ...
        if set(expected_columns).issubset(input_data.columns):
            for algo, model_file in zip(algonames, modelnames):
                model = pickle.load(open(model_file, 'rb'))
        
                # Ensure model only gets the original expected columns
                data_for_prediction = input_data[expected_columns].copy()

                # Make prediction
                input_data[algo + ' Prediction'] = model.predict(data_for_prediction)
...
with tab3:
    ...
    data = {'Decision Tree': 80.97, 'Logistic Regression': 85.86, 'Random Forest': 84.23, 'Support Vector Machine': 84.22, 'GridRF': 88.04}
